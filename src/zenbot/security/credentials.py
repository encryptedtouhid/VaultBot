...
SERVICE_NAME = "zenbot-agent"
_ENCRYPTED_STORE_DIR = Path.home() / ".zenbot" / "credentials"
...
class CredentialStore:
    ...
    def _ensure_encryption_key(self, passphrase: str | None = None) -> bytes:
        ...
        if passphrase is None:
            raise ValueError(
                ...
                "Set ZENBOT_VAULT_PASSPHRASE or use `zenbot credentials unlock`."
            )
    ...
    def check_for_plaintext_leaks() -> list[str]:
        ...
        """
        suspicious_paths = [
            Path.home() / ".zenbot" / "credentials.json",
            Path.home() / ".zenbot" / "credentials.yaml",
            Path.home() / ".zenbot" / "secrets.json",
            Path.home() / ".zenbot" / ".env",
