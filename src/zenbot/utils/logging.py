...
Log files:
    ~/.zenbot/logs/zenbot.log       — application log (all events)
    ~/.zenbot/logs/zenbot.error.log  — errors only
    ~/.zenbot/logs/audit.log         — security audit events
...
_LOG_DIR = Path.home() / ".zenbot" / "logs"
...
def setup_logging(
    *,
    ...
    Args:
        ...
        log_dir: Directory for log files. Defaults to ~/.zenbot/logs/.
...
def _setup_stdlib_logging(log_dir: Path, level: str) -> None:
    ...
    error_log = log_dir / "zenbot.error.log"
    ...
    audit_logger = logging.getLogger("zenbot.audit")
