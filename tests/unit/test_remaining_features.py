...
class TestAutoReply:
    def test_auto_reply_match(self) -> None:
        ...
        engine.add_reply_rule(
            AutoReplyRule(
                name="greeting", pattern=r"^(hi|hello)", response="Hello! How can I help?"
            )
        )
    ...
    def test_routing_rule(self) -> None:
        ...
        engine.add_routing_rule(
            RoutingRule(name="code", pattern=r"(code|function|bug)", target_model="deepseek-coder")
        )
    ...
    def test_disabled_rule_skipped(self) -> None:
        ...
        engine = AutoReplyEngine()
        engine.add_reply_rule(
            AutoReplyRule(name="r1", pattern="hi", response="hello", enabled=False)
        )
